...
OWNER_CHANNEL = os.getenv("OWNER_CHANNEL", "@ScienceStudyRoom")  # Replace with your channel
QUIZZORA_BOT = os.getenv("QUIZZORA_BOT", "@QuizzoraBot")
MAX_FREE_CHANNELS = int(os.getenv("MAX_FREE_CHANNELS", 2))
...
def check_subscription(client, message: Message):
    ...
    if not_subscribed:
        ...
        warning = message.reply_text(warning_text, reply_markup=InlineKeyboardMarkup(buttons))
        time.sleep(30)
        warning.delete()
...
def remove_channel_prompt(client, callback_query: CallbackQuery):
    chat_id = callback_query.from_user.id
    if chat_id not in force_sub_channels or not force_sub_channels[chat_id]:
        callback_query.message.edit_text("❌ No channels to remove!")
        return
    
    buttons = [[InlineKeyboardButton(channel, callback_data=f"remove_{channel}")]
               for channel in force_sub_channels[chat_id]]
    callback_query.message.edit_text("Select a channel to remove:", reply_markup=InlineKeyboardMarkup(buttons))
...
# Science Study Room
# This is a placeholder for the actual bot instance